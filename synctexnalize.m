% -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- --
    ...
    if you publish non - sense, so do not blame the author % -assume everything can be wrong unless you know what you are doing % -feel free to enhance or suggest enhancements % the script uses functionality provided by the mtex toolbox % (https : // mtex-toolbox.github.io/index)

                                                                                                                                                                   % TODO : % -undisort rings using the PONI file in case the detector normal was not% parallel to the beam % -speed up peak fitting option

                                                                                                                                                                   % written by Rüdiger Kilian(ruediger.kilian @geo.uni - halle.de)

                                                                                                                                                                   % % % addpath 'put mtex path here' %
                                                                                                                                                               < -change here % startup_mtex

                                                                                                                                                                   % % 0) general set up of input,
...
% < -change here
        % filename pattern,
    if just *, all the files in inpath infiles = dir([inpath 'experiments01_-*.tif']);
% < -change here

        % location of masks,
    backgounds etc.if needed
        synchrotronpath
    = '/path/to/additional/data/' % < -change here
...
bshm_image = [synchrotronpath 'mask_simple.tif'];
% < -change here
...
% default
    ...
    % wavelength
    % in meter,
    e.g.obtained from the poni file
        wavelength
    = 1.4234695572124025e-11;
...
%
{
    ...
    % compute distances rom center
            r_min_max
        = sqrt((center(1) - dvalue( :, 1)).^ 2 + (center(2) - dvalue( :, 2)).^ 2);
    hold on
        % check
        ...
        % ring in meters(2 theta)
            rim
        = mean(r_min_max) * pixSize;
    % 2theta twothetarim = atand(rim / sd);
    % wavelength
            dvalue
        = wavelength / (2 * sind(twothetarim / 2));
    %
}
...
% % % % option 2 : supply dvalue and range
    ...
    or define yourself
        % cs
    = loadCIF([CIFpath 'Kaolinite.cif'])
        %
    or cs
    = crystalSymmetry('1', [ 5.2, 8.9, 7.4 ], [ 91.7, 104.862, 89.822 ] * degree, 'X||a*', 'Z||c');
% Miller of interest
        h
    = Miller(0, 0, 1, cs);
...
% determine how wide a peak should be
        r_min_max
    = [crim + 5 crim - 5];       % <- this value needs to be set for automation
...
filelist= 1:
nfiles; % 1:2:nfiles % maybe one would only want to take every x-th image
for i=1:length(filelist) % do not read the last image from 0==180
    ...
    % _-- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -
    % % plotting and comparing data function tplot(data, n) subplot(1, 2, n) a_slice
    = mean(data, 3);
% a_slice = a_slice - min(a_slice( :));
ll = quantile(a_slice( :), 0.001);
% adapt values
        la
    = quantile(a_slice( :), 0.999);
% adapt values
        imagesc(a_slice, [ll la])
            axis equal
    end
    ...
    % _-- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -
    % % subtracting the background function data
    = bgSubtract(data, bgf, factor, varargin)

        if check_option (varargin, 'show'),
    tplot(data, 1);
end
...
if check_option (varargin, 'show')
    ...
    % _-- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -
    % % Fourier Filtering function outimage
    = FTpass(B, ll, ul, varargin)
    % data,
    lower limit, upper limit, (size of gaussian on fft mask) if check_option (varargin, 'show'), tplot(B, 1); end
...
if ndims(B) == 2
    ...
    % grid of u, v[V, U] = meshgrid(v, u);

    % distance from 0 frequency
            D
        = sqrt(U.^ 2 + V.^ 2);

    % determien filter unsing cut - off frequencies H = double(D >= ll & D <= ul);
    if
        ~isempty(varargin) & isnumeric(varargin { 1 }) H = imgaussfilt(H, varargin { 1 });
    end
...
b = [b(w : -1 : 1, :); b; b(end : -1 : end - w, :)];
    ...
    % _-- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -
    % % filter vertical stripes function imgs
    = filterVerticalStripes(imgs, varargin)
...
imgs( :, :, i) = d_temp;
    ...
    % _-- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -
    % % mask with mask function imgs
    = maskData(imgs, mask, varargin)
...
d_temp(bshmi) = nan;
imgs( :, :, i) = d_temp;
end if check_option (varargin, 'show'), tplot(imgs, 2);
end
        end

    % _-- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -
    % % extract the data function[pfvals, g]
    = rings2VecData(data, center, r_min_max, twothetarim, rotval, varargin)

    % prepare
        a_slice
    = mean(data, 3);
% get coordinates of eta - theta maps[pix_X, pix_Y] = meshgrid(1 : size(a_slice, 1), 1 : size(a_slice, 2));
% shift grid values relative to center
        pix_X
    = center(1) - pix_X;
% -center(1);
pix_Y = center(2) - pix_Y;
% -center(2);

pix_eta = atan2(pix_Y, pix_X) + pi;
% radial distance to select ring
        pix_theta
    = sqrt(pix_Y.^ 2 + pix_X.^ 2);
% circlular map to get data

    % % visualize
    % subplot(1, 2, 1)
    % imagesc(pix_eta / degree);
hold on
    % plot(center(1), center(2), 'rx');
% hold off
    % axis equal;
title('eta');
xlim([0 size(a_slice, 1)]);
ylim([0 size(a_slice, 2)])
    %
    % subplot(1, 2, 2)
    % imagesc(pix_theta);
hold on
...
ylim([0 size(a_slice, 2)])
        ...
        % define read
    - out eta - theta within rim

        % those are the ids of pixels within the ring id_theta
    = pix_theta < max(r_min_max) & pix_theta > min(r_min_max);
...
% get bins and ids eta bins such that we can have user - defined eta - intervals binspec = get_option(varargin, 'binSpec', 60);

if length (binspec)
    == 1 eta_bins = linspace(0, 2 * pi, binspec);
elseif length(binspec) > 1 & max(binspec) == 2 * pi& min(binspec) == 0;
eta_bins = binspec;
else disp('Please make sure your vector of bins goes from 0 to 2pi')
    end
        [~, ~, etabinId]
    = histcounts(eta_value, eta_bins);
...
% reduce to those in the rim
    ...
    for
        k = 1 : length(pfazi { i }) % loop over every binned / rounded eta

                % what goes into the k'th bin data val_k
            = tempdata(etabinId == k);

    % theta values
            theta_k
        = theta_value(etabinId == k);
    ...
    function li = lorentzIntegral(theta,data,limits)
        ...
        % compute coefficients
                p
            = lsqcurvefit(@lorentz, p0, theta, data, [], [], optimset('Display', 'off'));

        % do the integral
                li
            = integral(@(x)(p(1))./ ((x - p(2)).^ 2 + p(3)), min(r_min_max), max(r_min_max));
        end
...
if check_option (varargin, 'removeOutliers')
    % remove outliers(slow)
but maybe helpful
    % here numeric value is fraction of std
        olsigma
    = get_option(varargin, 'removeOutliers', 0.8);
% % ido = isOutlier(vu, pfv, olsigma);
% % vu(ido) = [];
pfv(ido) = [];
end
    function ind
    = isOutlier(v, val, facstd)
    % lifted from Mtex Polefigure / isOutlier
    % find neighbours
        next
    = find(v, v, 3 * v.resolution);
% remove diagonal
        next(speye(length(next)) == 1)
    = false;
% compute mean
        dmean
    = next * val./ sum(next, 2);
